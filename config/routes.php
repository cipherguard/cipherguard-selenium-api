 ...
 */
$routes->plugin('CipherguardSeleniumApi', ['path' => '/seleniumtests'], function ($routes) {
    $routes->setExtensions(['json']);

    if (Configure::read('cipherguard.plugins.selenium_api.security.endpoints.reset')) {
        $routes->connect('/resetInstance/{dataset}', ['controller' => 'ResetInstance', 'action' => 'resetInstance'])
            ->setPass(['dataset'])
            ->setMethods(['GET']);
    }

    if (Configure::read('cipherguard.plugins.selenium_api.security.endpoints.error')) {
    ...
    if (Configure::read('cipherguard.plugins.selenium_api.security.endpoints.email')) {
